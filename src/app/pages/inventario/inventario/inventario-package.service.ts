...
export class InventarioPackageService {

  private inventario = new BehaviorSubject<any[]>([]);
  public response$: Observable<any[]> = this.inventario.asObservable();
  ...
  popForm(data:any){
   ...
   mostrar(){
    ...
    const request$ = this._globals.obtener('inventario').pipe(tap((resp:any)=>{
    this.Cargando$.next(false);
     this.inventario.next(resp)
   }));
    return request$.subscribe();
  }
